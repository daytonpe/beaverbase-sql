...
public class BeaverBase {
    ...
    public static void parseQuery(String queryString) {
        ...
        if (fromString.contains("where"))
        {
            ...
            /*can't match on null value*/
            if (constraintValue.equals("null")) {
                System.out.println("\nNULL comparison is not supported at this time.\n");
                return;
            }
    ...
    public static void parseCreateTable(String createTableString) {
        ...
        ArrayList <String> split3 = new ArrayList<>(Arrays.asList(split1.get(1).trim().split(",")));


        /*if the first attribute isn't rowid and/or doesn't say PRIMARY KEY, throw*/
        String primKey = split3.get(0)
                .replace(")", "")
                .trim()
                .toLowerCase();
    ...
    public static void parseInsert(String insertString){
        ...
        ArrayList<String> initialInsertTokens = new ArrayList<>(Arrays.asList(insertString.split("values")));

        /*parse values*/
        String valueString = initialInsertTokens.get(1)
                .replaceAll("\\s+", "")
                .replace("(", "")
                .replace(")", "");
        valueList = new ArrayList<>(Arrays.asList(valueString.split(",")));

        String leftString = initialInsertTokens.get(0);
        ArrayList<String> split2 = new ArrayList<>(Arrays.asList(leftString.split("\\(")));

        /*parse tableName*/
        String tableString = split2.get(0).trim();
        ArrayList<String> tableArr = new ArrayList<>(Arrays.asList(tableString.split("\\s+")));
        String tableName = tableArr.get(2);

        /*parse columns*/
        String columnString = split2.get(1)
                .replaceAll("\\s+", "")
                .replace("(", "")
                .replace(")", "");
        columnList = new ArrayList<>(Arrays.asList(columnString.split(",")));
    ...
    public static void insertRecord(
        ...
        ArrayList<String> orderedValueList,
        ArrayList<String> isNullableList,
...
//                                System.out.println("\n\ndataType = " + dataType + " from recordPayloadPointer = "+recordPayloadPointer);
    ...
    public static boolean validateQueryColumns(
        ArrayList<String> columnList,
        ArrayList<String> columnListActual
        ){
        for (int i = 0; i < columnList.size(); i++) {
    ...
    public static ArrayList<Boolean> getBooleanPrintArray(
        ArrayList<String> columnList,
        ArrayList<String> columnListActual
        ){
    ...
    public static boolean checkIntConstraint(
        int value,
        int constraint,
        String operator
        ){
    ...
    public static boolean checkDoubleConstraint(
        double value,
        double constraint,
        String operator
        ){
    ...
    /*given two doubles and a constraint type, determine if the constraint is met*/
    public static boolean checkFloatConstraint(
        float value,
        float constraint,
        String operator
        ){
    ...
    public static boolean checkLongConstraint(
        long value,
        long constraint,
        String operator
        ){
    ...
    static void initializeDataStore() {
            ...
            try {
                    ...
                    /*write placeholder FF FF FF FF for the Right Page*/
                    beaverbaseColumnsCatalog.writeInt(-1);
